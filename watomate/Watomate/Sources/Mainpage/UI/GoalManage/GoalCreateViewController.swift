...
class GoalCreateViewController: PlainCustomBarViewController {
    ...
    @objc private func okButtonTapped() {
        let entry = GoalCreate(
            title: goalTitleView.text ?? "",
            visibility: goalVisibility?.rawValue ?? "PR",
            color: goalColor ?? "blue"
        )
        print(entry)
        viewModel.addGoal(entry) { result in
    ...
    override func viewDidLayoutSubviews() {
        ...
        goalTitleView.layer.addBorder(color: UIColor(named: "blue") ?? .systemGray4, width: 2.0)
    ...
    private lazy var goalCreateView: UIStackView = {
    ...
    private func setupLayout(){
        contentView.addSubview(goalCreateView)
        goalCreateView.snp.makeConstraints { make in
    ...
    private func updateGoalColor() {
        goalColorView.backgroundColor = UIColor(named: goalColor ?? "blue")

        goalTitleView.layer.addBorder(color: UIColor(named: goalColor ?? "blue") ?? .black, width: 2.0)
    }
